...
export let Splat = forwardRef(function Splat({ src, ...props }, ref) {
  ...
  useEffect(() => {
          ...
          if (
            ...
            if (bytesRemains > vertexCount * locals.rowLength) {
              const extra_data = new Uint8Array(
                bytesRemains - vertexCount * locals.rowLength
              );
              extra_data.set(
      ...
      if (bytesDownloaded - bytesProcesses > 0) {
        ...
        if (isPly)
          concatenatedChunks = new Uint8Array(
            processPlyBuffer(concatenatedChunks.buffer)
          );
        let numVertexes = Math.floor(
          concatenatedChunks.byteLength / locals.rowLength
        );
        if (!glInitialized) {
          await initGL(numVertexes);
          glInitialized = true;
    ...
    async function initGL(numVertexes) {
      ...
      locals.centerAndScaleData = new Float32Array(
        locals.bufferTextureWidth * locals.bufferTextureHeight * 4
      );
      locals.covAndColorData = new Uint32Array(
        locals.bufferTextureWidth * locals.bufferTextureHeight * 4
      );
      ...
      let splatIndexArray = new Uint32Array(
        locals.bufferTextureWidth * locals.bufferTextureHeight
      );
      ...
      while (true) {
        const centerAndScaleTextureProperties = gl.properties.get(
          locals.centerAndScaleTexture
        );
        const covAndColorTextureProperties = gl.properties.get(
          locals.covAndColorTexture
        );
        if (
    ...
    function pushDataBuffer(buffer, vertexCount) {
      ...
      const covAndColorData_uint8 = new Uint8Array(
        locals.covAndColorData.buffer
      );
      const covAndColorData_int16 = new Int16Array(
        locals.covAndColorData.buffer
      );
      ...
      while (vertexCount > 0) {
        ...
        let yoffset = Math.floor(
          locals.loadedVertexCount / locals.bufferTextureWidth
        );
        if (locals.loadedVertexCount % locals.bufferTextureWidth != 0) {
          width =
            Math.min(locals.bufferTextureWidth, xoffset + vertexCount) -
            xoffset;
          height = 1;
      ...
      worker.postMessage({ method: "push", matrices: matrices.buffer }, [
        matrices.buffer,
      ]);
    }
  ...
  useFrame((state, delta) => {
    ...
    if (locals.sortReady) {
      ...
      worker.postMessage(
        { method: "sort", view: view.buffer, cutout: undefined },
        [view.buffer]
      );
    }
  });
